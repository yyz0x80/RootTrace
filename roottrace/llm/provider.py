...
class LLMProvider:
    ...
    def __init__(
        ...
        if base_url is None:
            base_url = os.getenv("ROOTTRACE_BASE_URL")
        if not base_url:
            raise ValueError("ROOTTRACE_BASE_URL environment variable is not set")

        configured_model = model or os.getenv("ROOTTRACE_MODEL")
