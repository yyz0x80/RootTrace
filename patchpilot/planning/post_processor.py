...
def _clean_null_baseline_evidence(plan: ChangePlan) -> ChangePlan:
    """Convert null baseline_evidence values to empty strings.

    LLMs may return null for optional fields despite prompt instructions.
    This function converts null values to empty strings to prevent
    ...
    Returns:
        Modified plan with null baseline_evidence values converted to empty strings.
    """
    # Clean criterion_plans
    cleaned_criterion_plans = []
    for criterion_plan in plan.criterion_plans:
        if criterion_plan.baseline_evidence is None:
            cleaned_plan = criterion_plan.model_copy(
                update={"baseline_evidence": ""}
            )
            cleaned_criterion_plans.append(cleaned_plan)
        else:
            cleaned_criterion_plans.append(criterion_plan)

    # Clean verification_specs
    cleaned_verification_specs = []
    for verification_spec in plan.verification_specs:
        if verification_spec.baseline_evidence is None:
            cleaned_spec = verification_spec.model_copy(
                update={"baseline_evidence": ""}
            )
            cleaned_verification_specs.append(cleaned_spec)
        else:
            cleaned_verification_specs.append(verification_spec)

    return plan.model_copy(
        update={
            "criterion_plans": cleaned_criterion_plans,
            "verification_specs": cleaned_verification_specs,
        }
    )
...
def post_process_plan(
    ...
    1. Clean null baseline_evidence values to empty strings
    2. Migrate test files from planned_changes to planned_tests
    3. Merge duplicate file changes
    4. Validate pytest targets
    5. Check for ambiguous points
    6. Classify semantic ambiguities (currently no-op, relies on LLM)
    7. Validate criterion plans (if skip_ac_validation is False)
    8. Complete AC mapping validation (if skip_ac_validation is False)
