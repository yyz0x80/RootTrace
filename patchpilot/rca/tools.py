...
PatchPilot's ``Workspace``, ``ToolInput``/``ToolRegistry`` patterns, and the
existing ``search_code``/``read_file`` implementations.

The registry exposes exactly seven tools:

- ``search_code``, ``read_file``: reused from PatchPilot (search hardened).
...
Write-capable PatchPilot tools (``edit_file``, ``write_file``,
``write_scratch_test``, ``run_command``) are never registered, and their
handler methods raise so RCA Agents cannot mutate the target repository.
"""
...
from patchpilot.models import ToolFailureType
from patchpilot.rca.schema import validate_relative_path
from patchpilot.tools import ReadFileInput, SearchCodeInput, ToolInput, ToolRegistry
...
class RcaToolRegistry(ToolRegistry):
    ...
    Reuses PatchPilot's ``Workspace`` path/secret enforcement, ``ToolInput``
    schema generation, and the inherited ``search_code``/``read_file``
    implementations. Git-backed tools build allowlisted argv lists and run with
    ...
    # -- write-capable PatchPilot tools are never exposed to RCA Agents --------
