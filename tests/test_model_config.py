...
from patchpilot.model_config import ModelConfig, ModelConfigManager
...
def test_resolve_with_env_fallback_uses_env_when_config_not_found() -> None:
    ...
    with patch.dict("os.environ", {"ZHIPU_API_KEY": "env-key", "PATCHPILOT_BASE_URL": "https://env.com"}, clear=False):
...
def test_resolve_with_env_fallback_uses_default_model() -> None:
    ...
    # Clear PATCHPILOT_MODEL to test the default behavior
    with patch.dict("os.environ", {"ZHIPU_API_KEY": "env-key", "PATCHPILOT_BASE_URL": "https://env.com"}, clear=True):
        # Add back the required variables
        os.environ["ZHIPU_API_KEY"] = "env-key"
        os.environ["PATCHPILOT_BASE_URL"] = "https://env.com"

        model_id, base_url, api_key = manager.resolve_with_env_fallback(None, "openai")

        assert model_id == "gpt-4o"  # Default model when PATCHPILOT_MODEL is not set
        assert base_url == "https://env.com"
        assert api_key == "env-key"
